...
def today_in_lua():
    """Calculates the current Lua day and month"""
    lua_months = ["Wyrm", "Dance", "Milk", "Mead", "Wrath", "Dim", "Moth",
                   "Blood", "Veil", "Blooming", "Memory", "Bane", "Wake"]
    start_day = datetime.datetime(2016, 10, 13)
    index = 7
    new_moons_since = get_moons_in_timeframe(start=start_day, end=datetime.datetime.now(), moon_type="new")
    ...
    print("====================\n\n")
    days_since_start = datetime.datetime.now() - new_moons_since[i]
    print("Today's date: {} of {}".format(days_since_start.days + 1, this_month))
...
def create_lua_calendar(start, end):
...
if __name__ == "__main__":
    future_date = datetime.datetime(2025, 12, 30)
